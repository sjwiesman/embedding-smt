...
public final class RetryingEmbeddingClient {
  ...
  public RetryingEmbeddingClient(
      EmbeddingProvider provider, int maxRetries, long backoffMs, Sleeper sleeper) {
    ...
    while (true) {
      try {
        ...
        if (attempt >= maxRetries) {
          throw new RetriableException("Embedding request failed after " + attempt + " retries", e);
        }
        backoff(attempt);
        attempt++;
      }
