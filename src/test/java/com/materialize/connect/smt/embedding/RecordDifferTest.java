...
class RecordDifferTest {
  ...
  void detectsMultipleChangedColumns() {
    ...
    assertThat(RecordDiffer.changedColumns(before, after))
        .containsExactlyInAnyOrder("title", "views");
  }
  ...
  void newColumnsInAfterAreReportedAsChanged() {
    Schema beforeSchema = SchemaBuilder.struct().field("title", Schema.STRING_SCHEMA).build();
    Schema afterSchema =
        SchemaBuilder.struct()
            .field("title", Schema.STRING_SCHEMA)
            .field("body", Schema.STRING_SCHEMA)
  ...
  void removedColumnsAreReportedAsChanged() {
    Schema beforeSchema =
        SchemaBuilder.struct()
            ...
            .field("body", Schema.STRING_SCHEMA)
            .build();
    Schema afterSchema = SchemaBuilder.struct().field("title", Schema.STRING_SCHEMA).build();
